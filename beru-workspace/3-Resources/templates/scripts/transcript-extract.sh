...
#
# DEFAULT OUTPUT:
#   beru-workspace/0-Inbox/    (resolved relative to repo root, run from anywhere)
#
# DEPENDENCIES:
...
# Marker: presence of beru-workspace/0-Inbox/ relative to a directory means it's the repo root.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$SCRIPT_DIR"
while [ "$REPO_ROOT" != "/" ] && [ ! -d "$REPO_ROOT/beru-workspace/0-Inbox" ]; do
    REPO_ROOT="$(dirname "$REPO_ROOT")"
done

if [ "$REPO_ROOT" = "/" ]; then
    echo "❌ Could not locate repo root (looking for beru-workspace/0-Inbox/ ancestor)" >&2
    echo "   Run this script from inside the daily-agents tree, or pass output-dir explicitly." >&2
    exit 1
fi

URL="${1:-}"
OUTPUT_DIR="${2:-$REPO_ROOT/beru-workspace/0-Inbox}"

if [ -z "$URL" ]; then
    echo "Usage: $0 <youtube-url> [output-dir]"
    echo "Default output dir: $REPO_ROOT/beru-workspace/0-Inbox/"
    exit 1
fi
