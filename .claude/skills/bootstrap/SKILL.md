...
```bash
ls -d beru-workspace 2>&1                    # default workspace folder name
test -d beru-workspace/0-Inbox && echo OK    # canonical subdirs
test -d beru-workspace/1-Projects && echo OK
test -d beru-workspace/2-Coding/work && echo OK
test -d beru-workspace/2-Coding/personal && echo OK
test -d beru-workspace/2-Coding/forks && echo OK
test -d beru-workspace/2-Coding/archive && echo OK
test -d beru-workspace/3-Resources && echo OK
test -d beru-workspace/4-Archive && echo OK
```
...
| `workspace.root` | "Workspace folder name? [default: beru-workspace]" | default `beru-workspace` |
...
`AskUserQuestion`:
- **(a) Keep Beru default** — chief-of-staff persona, "🎯", professional-but-warm voice. Skill skips Phase 5 for SOUL/IDENTITY/USER (preserves Beru content), only updates Configuration section in CLAUDE.md with identity values.
...
If (a): record `assistant.name=Beru`, `assistant.role=Chief of Staff`, `assistant.vibe="Professional but warm — like a trusted exec assistant…"`, `assistant.emoji=🎯`.
...
```bash
ls beru-workspace/3-Resources/design-systems/ | grep -v '^README'
...
cp "beru-workspace/3-Resources/design-systems/<brand>/DESIGN.md" DESIGN.md
```
...
For each persona template at `beru-workspace/3-Resources/templates/persona/<file>-template.md`:
...
- `SOUL-template.md` → `SOUL.md` (only if Phase 3 = customize; else skip — keep Beru's voice)
- `IDENTITY-template.md` → `IDENTITY.md` (only if Phase 3 = customize)
- `USER-template.md` → `USER.md` (always — even keeping Beru, swap `<user.name>`'s specifics for new user's)
...
If the user renamed `workspace.root` from default `beru-workspace`: also update CLAUDE.md's Configuration section's `workspace.root` value (deferred to Phase 8 for atomicity).
...
- user.github: <value>
- assistant.name: <value> (default: Beru / customized)
- workspace.root: <value>
- design-system: <brand name>
- Persona files regenerated: <list, or "kept Beru default">
