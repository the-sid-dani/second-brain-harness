---
name: jira-create-vertical-slices
description: Bulk-create N Jira Story tickets under an existing Epic, all sharing a consistent shape — header info panel with Confluence + repo cross-links, ADF taskList for acceptance criteria (interactive checkboxes), parent linkage, custom fields (Team, AI Category, label), and Blocks links wired between sequential slices. Use this whenever <user.name> wants to break an Epic into vertical slices and create the corresponding Jira tickets in one batch — phrases like "create the slices", "scaffold the stories under <Epic>", "spin up Slices A through G", "decompose this Epic into stories", "bulk-create these vertical slices in Jira", or any time a design doc lists multiple slices/stories that need to land in Jira together. Trigger broadly on bulk-Story-creation language under an existing Epic. Reads from a YAML or JSON spec file the user prepares; does NOT create the spec from a conversation — the user (or another upstream skill) writes the spec, this skill executes it. Inputs go in, tickets come out, links wire up.
---
...
2. Confirm `epic_key` exists via `GET /rest/api/3/issue/{key}` (basic-auth from `~/.daily-agents.env`)
...
The script reads `ATLASSIAN_BASIC_AUTH` from the environment (auto-loaded via `~/.zshrc` → `~/.daily-agents.env`). If the env var isn't set, the script tells the user to source the env file or run from a fresh shell.
