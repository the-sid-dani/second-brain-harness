---
name: save-resource
description: Saves a file or content into `<workspace.root>/<workspace.resources>/<type>/...` with the right convention per type — research goes to `YYYY-MM-<topic>-research/<file>` (date-prefixed topic folder), reference goes flat as `<file>`, meetings goes to `YYYY-MM-DD-<topic>.md`, custom types go to `<custom>/<file>`. All paths come from the Configuration section in root CLAUDE.md — read those first. Use this whenever the user wants to save something into Resources — phrases like "save this to research", "file this in reference", "this is a meeting note", "promote this to resources", "save the X file in inbox to research/reference/meetings", "this belongs in <type>", "/save-resource". Trigger even when the user doesn't say "save-resource" explicitly — saving / filing / promoting reference material should fire this rather than letting them `mv` by hand (which would skip the type-aware destination logic). Handles three sources: file path in invocation, generic Inbox reference (lists Inbox candidates and asks), or chat content (writes new file). Replaces the planned `/promote-resource` skill — broader scope, handles both direct save AND Inbox→Resources promotion.
---
