# Daily Agents — Project Instructions for <assistant.name>
...
### workspace
- `workspace.root` = `<workspace.root>` (default: `beru-workspace`; rename to `workspace`, `brain`, `<assistant>-workspace`, etc.)
...
Resolved paths (combine the above): `<workspace.root>/<workspace.projects>` → e.g., `beru-workspace/1-Projects`.
