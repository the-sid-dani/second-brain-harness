...
Local Model Context Protocol servers for <agent>. These extend Claude Code with capabilities that aren't available in the base toolset.

## gemini-vision

Lets Claude actually *see* images, PDFs, documents, and videos via Google's Gemini API. Without this, screenshots / PDFs dropped into `0-Inbox/` are opaque to <agent> — the MCP turns them into structured analysis.
...
Free tier (15 req/min) is fine for the volume <agent> will use this at — image triage on inbox items, occasional PDF extraction, the odd YouTube transcript. Paid tier exists if you need more throughput, but unlikely.
