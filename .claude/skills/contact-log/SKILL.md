---
name: contact-log
description: Write-mode skill — append a new interaction entry to a matched contact's `## Interaction log` section in `<workspace.root>/<workspace.resources>/contacts/<slug>.md` and auto-bump the `last_interaction` frontmatter field to today's date. Append-only; never rewrites prior entries. Use whenever <user.name> wants to record an interaction with someone in contacts/ — phrases like "/contact-log <name>", "log my meeting with X", "log my 1:1 with Y", "append interaction with Z", "I just talked to <name>", "just spoke with <name>, log it", "record my call with <name>". Trigger on write-mode entity-keyed phrasing — verbs like log/append/record/wrote/talked-to are the signal. Composes the same fuzzy-match priority order as `/contact` (locked: exact slug > slug-stem-match > frontmatter `name` token-level exact > frontmatter `legal_name` token-level exact > prefix on slug or `name` > substring on slug or `name`); for consistency the matching logic is reused — invoke `/contact <name>` to identify the target, then this skill handles the write. MUTATION INVARIANT: this skill MUST use the `Edit` tool to insert the new entry and bump frontmatter; it MUST NOT use `Write` (which would replace the entire file and could destroy prior interaction entries, About / Recurring topics / Open commitments sections). Only two fields mutate — the `last_interaction` frontmatter line and one new `### YYYY-MM-DD — <topic>` block under `## Interaction log`. SINGLE-SOURCE INVARIANT: writes ONLY to the contact file. Does NOT also append to `memory/YYYY-MM-DD.md` (the daily log) — daily log captures the day's narrative; per-person commitments live in contacts/. WikiLinks (`[[contacts/<slug>]]`) cross-reference between them. NO-FABRICATION INVARIANT: if <user.name> gives sparse input ("nothing happened", "just a quick chat"), accept it and write a minimal entry; never invent commitments or topics. Sister skills: `/contact` (read), `/contact-add` (scaffold new — planned), `/contact-update` (field edit — planned).
context: fork
---
